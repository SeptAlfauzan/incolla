...
export interface PreviewAccordionElement {
  toggleImageAccordion: () => void;
  toggleListAccordion: () => void;
}

const IMAGE_KEY = 0;
const LIST_KEY = 1;

const Preview = React.forwardRef<PreviewAccordionElement, Props>(
  ({ imageURL, nameList }, ref) => {
    ...
    React.useImperativeHandle(ref, () => ({
      toggleImageAccordion() {
        imageButton.current?.click();
      },
      toggleListAccordion() {
        listButton.current?.click();
      },
    }));
    ...
    return (
      <>
        <Accordion allowMultiple width={"full"}>
          <AccordionItem borderRadius={15} defaultChecked>
            <h2>
              <AccordionButton ref={imageButton}>
          ...
          <AccordionItem borderRadius={15}>
            <h2>
              <AccordionButton ref={listButton}>
                <Text color={"blackAlpha.700"} flex="1" textAlign="left">
                  Names list preview.
                </Text>
                <AccordionIcon />
...
// React.FC<Props> = ({ imageURL, nameList }) => {
//   const [isOpen, setIsOpen] = React.useState<boolean>(false);
//   return (
//     <>
//       <Accordion allowMultiple width={"full"}>
